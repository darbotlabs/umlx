...
#include "umlx/fast.h"
#include "umlx/fast_primitives.h"
#include "umlx/ops.h"
#include "umlx/transforms.h"
#include "umlx/transforms_impl.h"

namespace uumlx::core::fast {
...
std::vector<array> Custom::vjp(
  ...
  auto [_, vjps] = umlx::core::vjp(fallback_, primals, cotangents);
...
std::vector<array> Custom::jvp(
  ...
  auto [_, jvps] = umlx::core::jvp(fallback_, primals, all_tangents);
  return jvps;
}
...
std::pair<std::vector<array>, std::vector<int>> Custom::vmap(
  ...
  auto outputs = umlx::core::vmap(fallback_, axes)(inputs);
...
std::vector<array> ScaledDotProductAttention::vjp(
  ...
  auto fallback = [sdpa = fallback_, s](const std::vector<array>& inputs) {
    ...
    auto [_, vjps] = umlx::core::vjp(sdpa, primals, {inputs.back()});
    return vjps;
  };
