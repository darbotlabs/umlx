...
#include "umlx/backend/common/compiled.h"
#include "umlx/backend/cuda/device.h"
#include "umlx/backend/cuda/jit_module.h"
#include "umlx/backend/cuda/kernel_utils.cuh"
...
#include "umlx/backend/cuda/device/binary_ops.cuh"
#include "umlx/backend/cuda/device/ternary_ops.cuh"
#include "umlx/backend/cuda/device/unary_ops.cuh"
#include "umlx/backend/cuda/device/utils.cuh"
...
void Compiled::eval_gpu(
  ...
  cu::JitModule& mod = cu::get_jit_module(s.device, lib_name(), [&]() {
    ...
    kernel_names.push_back(fmt::format(
        "umlx::core::cu::{}_contiguous<uint32_t, {}>",
        lib_name(),
        work_per_thread));
    kernel_names.push_back(fmt::format(
        "umlx::core::cu::{}_contiguous<int64_t, {}>",
        lib_name(),
        work_per_thread));
    for (auto wpt : std::array<int, 2>{1, work_per_thread}) {
      for (int i = 1; i <= MAX_NDIM; ++i) {
        kernel_names.push_back(fmt::format(
            "umlx::core::cu::{}_strided<{}, uint32_t, {}>", lib_name(), i, wpt));
        kernel_names.push_back(fmt::format(
            "umlx::core::cu::{}_strided<{}, int64_t, {}>", lib_name(), i, wpt));
      }
    }
  ...
  std::string kernel_name = fmt::format("umlx::core::cu::{}", lib_name());
  if (contiguous) {
    kernel_name +=
