...
.. currentmodule:: umlx.nn.init

Initializers
------------

The ``umlx.nn.init`` package contains commonly used initializers for neural
network parameters. Initializers return a function which can be applied to any
input :obj:`umlx.core.array` to produce an initialized output.
...
To re-initialize all the parameter in an :obj:`umlx.nn.Module` from say a uniform 
